...
import { computeStackTrace, StackFrame as TraceKitStackFrame } from './tracekit';
...
export function exceptionFromError(ex: Error): Exception {
  ...
  const stacktrace = computeStackTrace(ex);
...
export function extractType(ex: Error & { message: { error?: Error } }): string | undefined {
  ...
  if (!name && isWebAssemblyException(ex)) {
    ...
    return hasTypeInMessage ? (ex.message as unknown as string[])[0] : 'WebAssembly.Exception';
  }
...
export function extractMessage(ex: Error & { message: { error?: Error } }): string {
  ...
  if (isWebAssemblyException(ex)) {
    ...
    if (Array.isArray(ex.message) && ex.message.length == 2) {
      return (ex.message as unknown as string[])[1];
    }
...
function eventFromPlainObject(
  ...
  if (errorFromProp) {
    return {
      exception: {
        values: [exceptionFromError(errorFromProp)],
      },
      extra,
    };
  }
...
function eventFromError(ex: Error): Event {
  return {
    exception: {
...
export function eventFromString(
  ...
  if (attachStacktrace && syntheticException) {
    ...
    if (frames.length) {
      event.exception = {
        values: [{ value: message as string, stacktrace: { frames } }],
      };
    }
